...
export const api = {
  ...
  scanItem: (key, { barcode, clientEventId, deviceId, quantity }) => requestWithRetry(`/orders/${encodeURIComponent(key)}/items/scan`, { method: 'POST', body: JSON.stringify({ barcode, clientEventId, deviceId, quantity }) }),
  ...
  skipCheck: (key, expectedVersion) => request(`/orders/${encodeURIComponent(key)}/skip-check`, { method: 'POST', body: JSON.stringify({ expectedVersion }) }),
  approveManualPick: (key, lineNo) => request(`/orders/${encodeURIComponent(key)}/items/${lineNo}/approve-manual-pick`, { method: 'POST', body: JSON.stringify({}) }),
  pendingManualPickApprovals: () => request('/manual-pick-approvals/pending'),
  ...
  myShortages: (params = {}) => {
    const qs = new URLSearchParams(params).toString();
    return request('/inventory/my-shortages' + (qs ? `?${qs}` : ''));
  },
  shortedItems: () => request('/inventory/shorted-items'),
